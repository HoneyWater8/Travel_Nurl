...
from models.record import PhotoMetadata
...
class RecordService:
    ...
    async def upload_photo(self, user_id: str, file: UploadFile):
        ...
        with open(temp_file_path, "rb") as data:
            self.record_blob_client.upload_blob(f"{user_id}/{file.filename}", data)

        timestamp = datetime.utcnow()
        metadata = PhotoMetadata(filename=file.filename, timestamp=timestamp)  # type: ignore # file name 나중에 설정 로직 자기
        ...
        await search_history_collection.insert_one(mongo_record)
